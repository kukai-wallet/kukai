...
export class AccountTools extends ActivityTools {
  generateAccount(pkh: string, pk: string, xtzrate: number = 0.510272, balance: number = this.getEmptyBalance()): ImplicitAccount {
  ...
  generateAccounts(pkh: string[], pk: string[], activities: Activity[][] = []): ImplicitAccount[] {
    ...
    for (let i = 0; i < pkh.length; i++) {
      accounts.push(this.generateAccount(pkh[i], pk[i], 0.510272, this.generateBalance()));
    }
    return accounts;
  }
...
export class WalletTools extends AccountTools {
  ...
  generateWalletV1(
    ...
    originatedAccounts: string[] = ['KT1EyxYE9TWEVSvUktBMErqKfNxGEomTdoaf', 'KT1TZeMxMUHeLM4SvbcsvdpAG2KRHUYV7fiT'],
    xtzrate: number = 0.510272,
    balance: number = 100
  ): LegacyWalletV1 {
    const v1wallet = new LegacyWalletV1('U2R9zKaKW6EjngeL', seed);
    v1wallet.XTZrate = xtzrate;
    v1wallet.implicitAccounts.push(new ImplicitAccount(implicitAccount.pkh, implicitAccount.pk));
    v1wallet.implicitAccounts[0].balanceXTZ = balance;
    for (const origAcc of originatedAccounts) {
      v1wallet.implicitAccounts[0].originatedAccounts.push(new OriginatedAccount(origAcc, implicitAccount.pkh, implicitAccount.pk));
      v1wallet.implicitAccounts[0].originatedAccounts[v1wallet.implicitAccounts[0].originatedAccounts.length - 1].balanceXTZ = balance;
    }
    const { balanceXTZ, balanceUSD } = this.getWalletBalance(v1wallet.implicitAccounts);
  ...
  generateWalletEmbedded(): EmbeddedTorusWallet {
    ...
    wallet.XTZrate = 4.01;
    wallet.implicitAccounts.push(new ImplicitAccount(keyPair.pkh, keyPair.pk));
  ...
  generateWalletV2(
    ...
    implicitAccount: any = {
      pk: 'edpku7vd7gzLzeTwdo21VC97Vp23rTjpyAZxdXTWNWi66puhPuBRLe, pkh: tz1NUXpLwA9Jj5hXHZanYCV1pMNycxXx6zAf'
    },
    originatedAccounts: string[] = ['KT18e6uHg65SZUF2Gkh8GU2RRDaHm3VGfLhF', 'KT1LmKuo4GwJeRLtrZcdX2sSQyHAPZUHSb9T'],
    xtzrate: number = 0.510272,
    balance: number = 100
  ): LegacyWalletV2 {
    const v2wallet = new LegacyWalletV2('ce7738762322795aca89a178a5db915e', seed);
    v2wallet.XTZrate = xtzrate;
    v2wallet.implicitAccounts.push(new ImplicitAccount(implicitAccount.pkh, implicitAccount.pk));
    v2wallet.implicitAccounts[0].balanceXTZ = balance;
    for (const origAcc of originatedAccounts) {
      v2wallet.implicitAccounts[0].originatedAccounts.push(new OriginatedAccount(origAcc, implicitAccount.pkh, implicitAccount.pk));
      v2wallet.implicitAccounts[0].originatedAccounts[v2wallet.implicitAccounts[0].originatedAccounts.length - 1].balanceXTZ = balance;
    }
    const { balanceXTZ, balanceUSD } = this.getWalletBalance(v2wallet.implicitAccounts);
  ...
  generateHdWallet(
    ...
    implicitAccounts: any = [
      ...
      {
        pk: 'edpkvaNoKcTrQ8jBVHkVUzwZnLAaZT98ALxucqcfmkPAWGXuRVM9Db',
        pkh: 'tz1dXCZTs4pRTVvoXJXNRUmrYqtCde4fdP8N'
      }
    ],
    xtzrate: number = 0.510272,
    balance: number = 100
  ): HdWallet {
    const hdWallet = new HdWallet(iv, encryptedSeed, encryptedEntropy);
    hdWallet.XTZrate = xtzrate;
    for (const impAcc of implicitAccounts) {
      hdWallet.implicitAccounts.push(new ImplicitAccount(impAcc.pkh, impAcc.pk));
      hdWallet.implicitAccounts[hdWallet.implicitAccounts.length - 1].balanceXTZ = balance;
    }
    const { balanceXTZ, balanceUSD } = this.getWalletBalance(hdWallet.implicitAccounts);
