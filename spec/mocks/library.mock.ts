...
export class Tools {
	...
	generateRandomNumber(min: number, max: number, multiplier: number = 1, fixed: number = 0 ): number {
		...
		if (fixed) {
			return Number((Math.random() * (max - min) + min).toFixed(fixed)); } else {
	...
	dateDiffInDays(a: Date, b: Date) {
		...
		if (a === b) { console.log('they are the same'); return 0; } else {
	...
	printAccounts(accounts: Account[]): void {
		...
		for ( let i = 0; i < accounts.length; i++ ) {
			const pkh = accounts[i].pkh;
			const balance = ((accounts[i].balance.balanceXTZ) / 1000000).toFixed(2);
			const delegate = accounts[i].delegate;
			const numberOfActivites = accounts[i].numberOfActivites;
...
export class WalletTools extends AccountTools {
	...
	getWalletBalance( accounts: Account[] ): Balance {
		...
		if ( accounts.length ) {
			for ( let i = 0; i < accounts.length; i++ ) {
				const balance = accounts[i].balance.balanceXTZ;
				const pending = accounts[i].balance.pendingXTZ;
	...
	generateWallet(seed: string, type: WalletType= 0, accountpkhs: string[]= [], xtzrate: number= 0.510272, balance: Balance= this.getEmptyBalance()): Wallet {
		const accounts: Account[] = this.generateAccounts(accountpkhs);
